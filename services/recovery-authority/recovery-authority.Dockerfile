...
RUN CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o /out/recovery-authority ./cmd/recovery-authority
...
COPY --from=build /out/recovery-authority /usr/local/bin/recovery-authority
USER 10001:10001
ENTRYPOINT ["/usr/local/bin/recovery-authority"]
