...
| **ADR-028** | **Audit Reconciliation** | **Accepted** | EMG Founder | EMG Founder | Project Architect | **2026-08-09** | `EMG_ADR-028_AUDIT_RECONCILIATION.md` | **Implementation authorized; not implemented** — governs a dedicated, single-replica RC1 Audit Projector consuming tenant-partitioned `mutation_dispatch` audit work with tenant-scoped Service Principals, at-least-once delivery, bounded durable retry, idempotent acceptance, crash recovery, graceful shutdown, and backlog observations. Depends on ADR-027, ADR-030, ADR-034 |
...
| **ADR-041** | Production Provisioning Ownership & Bootstrap Contract | **Accepted** | EMG Founder | EMG Founder | Project Architect | 2026-08-09 | `EMG_ADR-041_PRODUCTION_PROVISIONING_OWNERSHIP_AND_BOOTSTRAP_CONTRACT.md` | **Implementation authorized; not implemented** — assigns the Platform Database Bootstrap Administrator, establishes `emg_audit_migrator` as Audit object/migration owner, governs `emg_audit_projector` and V007 ordering, defines one environment-owned non-secret projector identity inventory as the source for Keycloak, runtime credential mapping, and the Audit allow-list, and requires ordered fail-closed production finalization. |
