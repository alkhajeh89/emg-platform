...
## Architecture Decision Records (ADR-014 – ADR-045)

Thirty-one ADRs are present in this range. ADR-031 is intentionally absent;
...
| **ADR-045** | Recovery Authority: Signing Administrative Independence and Key-Lifecycle Protocol Extension | **Accepted (2026-08-17) — accepted after a final focused independent review of corrected text (`ACCEPT_AS_CORRECTED`, zero remaining P0/P1/P2); two prior review passes found and the document text corrected: a missing verification trust anchor (`CONTENT_BINDING != KEY_AUTHORIZATION != ACCEPTANCE_PROVENANCE`); a circular digest/signing construction order; and a factual error, found via live fact-checking against current official Cloud KMS documentation, requiring mandatory public-key pinning rather than assumed live post-disablement retrievability** | EMG Founder | EMG Founder | Project Architect | 2026-08-17 | `EMG_ADR-045_RECOVERY_AUTHORITY_SIGNING_ADMINISTRATIVE_INDEPENDENCE_AND_KEY_LIFECYCLE_PROTOCOL_EXTENSION.md` | **Resolves the two items ADR-044 §15A and §17 (items 9–10) explicitly reserved as a separate, subsequent implementation-review decision**: selects an independently administered GCP-native signing trust domain (ADR-044 §15A "Option A"), rejects a same-organization-only construction as insufficient to close the row-G organization-administrator threat, and specifies an additive `CommittedPayload` V2 protocol extension (`signing_key_id`, cryptographically bound into the canonical digest, under a new, additive `EMG-ADR044-COMMITTED-V2` domain separator) enabling historical signature verification across key rotation via mandatory public-key pinning (§7C) rather than live provider retrieval. Distinguishes routine key rotation from compromise revocation via a governed, administratively independent distrust-effective-time ledger compared against each record's own trusted commit timestamp. Authorizes `recovery/verifier.go`'s dead `CommittedCheckpointVerifier` interface for removal. **Does not create any GCP resource, IAM binding, or credential.** Now authorizes S1 (the protocol/interface code change it specifies) to begin. Production approval: `NOT ESTABLISHED`, unaffected |
