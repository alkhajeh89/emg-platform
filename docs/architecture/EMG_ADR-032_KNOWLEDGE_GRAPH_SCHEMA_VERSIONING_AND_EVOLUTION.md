...
*   **Backup and point-in-time recovery — ungoverned dependency.** Backups must include the schema version metadata to allow for accurate point-in-time recovery to the correct ontology version. **No accepted decision governs this requirement.** This bullet previously cited "ADR-031"; a repository-wide search on 2026-08-03 confirmed that **no ADR-031 document exists under any identifier or filename**, and that the citation was the only occurrence in the repository. ADR-017 §5 governs disaster-recovery *strategy* (recovery baseline, RPO/RTO prioritization) but governs neither backup mechanics, PITR procedure, retention, nor schema-version metadata in backups. This requirement therefore remains an **open governance dependency requiring a future architecture decision**, tracked in `EMG_ARCHITECTURE_DECISION_REGISTER.md` as **D-A-005**. No ADR-031 is authored by this correction.
