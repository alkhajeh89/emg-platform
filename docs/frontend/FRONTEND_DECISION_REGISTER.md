...
| D-F-006 | BFF Integration | **Accepted (2026-08-03)** | EMG Founder | Project Architect | **ADR-036** (`docs/architecture/EMG_ADR-036_APPLICATION_BFF_BOUNDARY.md`); ADR-014; ADR-034; ADR-038 | **Closed.** A BFF is mandatory; the browser never holds a Service Principal credential; the BFF is not a PEP. ADR-038 resolves the former downstream human-delegation sub-decision in ADR-036 §5. Acceptance of ADR-038 does not imply implementation; Phase 2B remains blocked until mandatory capability verification succeeds. |
