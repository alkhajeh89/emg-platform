"""Reusable identifier/label validation for emg-platform-core.

Identical policy to the sibling libraries (``emg_memory_graph.labels``,
``emg_knowledge_lifecycle.labels``, ``emg_semantic_layer``): every
externally-supplied identifier flows through ``ensure_safe_label``, which rejects
empty/whitespace-only strings and any NUL, ASCII-control, CR/LF, or Unicode
bidirectional override/control character while preserving legitimate Unicode.
Character allow/deny only — no regex execution, no expression parsing — so there
...
def ensure_safe_label(value: str) -> str:
    """Return ``value`` unchanged if it is a safe identifier/label; otherwise
    raise ``ValueError``. Rejects empty/whitespace-only strings and any NUL,
    ASCII control, CR/LF, or Unicode bidi override/control character."""
